...
<body>
	...
	<div id="container" class="clear">
		...
		<div class="adress_choice">
			...
			<c:forEach items="${arrList}" var="arr">
				<c:if test="${arr.isDefault==1}">
					<div id="addresId1" class="base_select">
				</c:if>
				<c:if test="${arr.isDefault==0}">
					<div id="addresId2" class="base">
				</c:if>
				<i class="address_name"> ${arr.recvName} </i>
				<i class="user_address">
					${arr.recvDistrict}${arr.recvAddr}${arr.recvPhone}</i>
		...
		<div class="product_confirm">
			...
			<c:forEach items="${cartList}" var="cart">
				<ul class="item_detail">
					<li class="p_info"><b><img src="..${cart.image}" width="84px;" height="84px;" /></b> <b
						class="product_name lf">${cart.title}</b> <br /> <span
						class="product_color "> 颜色：深空灰 </span></li>
					<li class="p_price"><i>达内专属价</i> <br /> <span
						class="pro_price">￥<span class="ppp_price">${cart.price}</span></span>
					</li>
					<li class="p_count">X<span>${cart.count}</span></li>
	...
	<script>
		$(".go_pay").click(function() {
			location.href = "../order/payment.do";
		})
	</script>
	<script>
