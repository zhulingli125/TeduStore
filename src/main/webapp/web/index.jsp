		...
		<div class="lf1_bottom">
			<div class="item_cat lf">
				...
				<div class="item_cat_all">
					<c:forEach items="${list1}" var="gc" varStatus="s">
						<p>${gc.name}</p>
						<ul>
							<c:forEach items="${list2[s.index]}" var="g">
								<li><a href="${pageContext.request.contextPath}/goods/showGoods.do?categoryId=${g.id}">${g.name}</a></li>
							</c:forEach>
			...
			<c:forEach items="${glist}" var="gl">
				<div class="item_msg lf">
					<img width="200px" height="140px" src="${pageContext.request.contextPath}${gl.image}" alt="${title}" />

					<p class="bottom_ys2">${gl.title}</p>

					<p class="bottom_ys3">￥${gl.price}</p>
